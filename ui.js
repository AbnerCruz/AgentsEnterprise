...
(function (S) {
  ...
  function pintarAmbienteBar(){
    ...
    box.innerHTML=`<span class="env-chip">🧱 ${objs.length} construções</span><span class="env-chip">🪙 ${F.num(a.moedas||0)} Cr do ambiente</span><span class="env-chip">${esc(a.tema||'oficina')}</span>`;
  }
  ...
  function painelPessoa(id) {
    ...
    folha(`
      ...
      <div class="stats">
        ...
        <div class="stat"><span>Salário interno</span><b>${F.num(f.salario&&f.salario.mensalCreditos||0)} Cr/mês</b></div>
        <div class="stat"><span>Carteira pessoal</span><b>${F.num(f.salario&&f.salario.saldoCreditos||0)} Cr</b></div>
  ...
  function pintarProjetos() {
    ...
    $('#projetosList').innerHTML = projetos.length ? projetos.map(pr => {
      ...
      const finais = arquivos.filter(a=>a.classe==='produto'&&a.clienteVisivel);
      ...
      return `<div class="item project-item">
        ...
        <div class="item-body">
          ...
          <div class="stats project-stats">
            ...
            <div class="stat"><span>Artefatos de produção</span><b>${arquivos.length}</b></div>
            <div class="stat"><span>Produtos finais</span><b>${finais.length}</b></div>
          </div>
          ${atividade.length ? `<div class="project-feed">${atividade.map(a=>`<div><span>${F.hora(a.t)}</span>${esc(a.texto)}</div>`).join('')}</div>` : ''}
          ${finais.length?`<div class="row-actions"><button class="btn btn-mini" data-projeto-zip="${pr.id}" type="button">Baixar release do cliente .zip</button></div>`:''}
        </div>
      </div>`;
  ...
  function pintarPendencias() {
    const e = S.state.atual(); if (!e) return;
    const cands = e.arquivos.filter(a => ['esboco','prototipo','candidato'].includes(a.classe) && !a.avaliado).slice(0, 6);
    const painel = $('#aprovacoesPanel');
    painel.hidden = cands.length === 0;
    if (!cands.length) return;
    painel.innerHTML = `<div class="panel-head"><span class="panel-label">Revisões do pipeline</span><span class="chip">${cands.length}</span></div>
      ${cands.map(a => `<div class="item">
        <span class="dot-state feita"></span>
        <div class="item-body">
          <div class="item-title">${esc(a.nome)}</div>
          <div class="item-meta"><span>${esc(ROTULO_CLASSE[a.classe]||a.classe)}</span><span>· ${a.clienteVisivel?'destino cliente':'interno'}</span><span>· por ${esc(a.autor)}</span></div>
        </div>

      </div>`).join('')}
      <p class="panel-foot">A gerente acompanha cada etapa. Produtos para cliente não podem saltar esboço → protótipo → candidato final; apenas o candidato limpo atravessa o gate de release. Materiais internos são aceitos no acervo sem virar produto.</p>`;
  }
  ...
  const EXPLICA_CLASSE = {
    todos: 'Pipeline obrigatório de produto: esboço → protótipo → candidato final → produto. Nenhum atalho publica uma etapa anterior.',
    esboco: 'Primeira materialização concreta. Existe para descobrir forma, conteúdo e lacunas; nunca é contato com o cliente.',
    prototipo: 'Versão completa o bastante para testar e revisar. Ainda pode mudar e não é uma entrega externa.',
    candidato: 'Versão finalizada para o cliente. O gate bloqueia notas internas, placeholders, status editoriais e etapas não comprovadas.',
    produto: 'Release congelado: é exatamente o arquivo que pode chegar ao cliente. Qualquer mudança exige uma nova versão e um novo pipeline.'
  };
  ...
  function pintarArquivos() {
        ...
        <div class="file-head">
          <div class="file-info">
            ...
            <div class="file-meta">
              ...
              <span class="quality">${a.classe==='produto'?'cliente final':a.classe==='candidato'?(a.validacao?.pronto?'gate local ok':'finalização pendente'):(a.validacao?.pronto?'estrutura ok':'em trabalho')}</span>
    ...
    $$('#filesList [data-publicar]').forEach(b => b.onclick = () => {
      const p = S.studio.publicar(b.dataset.publicar, 'você', 'release solicitado pelo dono');
  ...
  function produtosFinaisDoProjeto(e,projectId){
    const produtos=(e.arquivos||[]).filter(a=>a.projectId===projectId&&a.classe==='produto'&&a.clienteVisivel);
    const maisRecentes=new Map();
    produtos.forEach(a=>{const chave=a.linhagem||a.nome,atual=maisRecentes.get(chave);if(!atual||(a.versao||1)>(atual.versao||1)||(a.publicadoEm||0)>(atual.publicadoEm||0))maisRecentes.set(chave,a);});
    return Array.from(maisRecentes.values());
  }
  function exportarProjetoZip(projectId){
    const e=S.state.atual();if(!e)return;const pr=e.projetos.find(p=>p.id===projectId)||e.projetos.find(p=>p.status==='ativo')||e.projetos[0];if(!pr)return;
    const arqs=produtosFinaisDoProjeto(e,pr.id);
    if(!arqs.length)return toast('Ainda não existe release final deste projeto. Esboços, protótipos e materiais internos não entram no pacote do cliente.','erro');
    const pasta=S.util.slug(pr.nome);
    S.arquivo.baixarBlob(S.arquivo.zip(arqs.map(a=>({nome:`${pasta}/${a.nome}`,conteudo:a.conteudo}))),pasta+'-release.zip');
    toast(`${arqs.length} arquivo(s) final(is) empacotado(s). Nenhum material de produção foi incluído.`,'ok');
  }
  ...
  function exportarZip() {
    ...
    arquivos.push({
      ...
      conteudo: `# ${e.nome}\n\n${e.missao}\n\nRamo: ${e.ramo}\nPúblico: ${e.publico}\nExportado em ${new Date().toLocaleString('pt-BR')}\n\n## Arquivos\n\n` +
        ...
        `\n\nEste ZIP é um BACKUP DO ACERVO e pode incluir esboços, protótipos e documentos internos. Não trate este pacote como entrega ao cliente. Para uma entrega externa, use “Baixar release do cliente .zip” no projeto; esse pacote contém somente produtos finais publicados.\n`
    });
  ...
  function pintarEconomia(){
    const e=S.state.atual(); if(!e || !S.economia) return;
    const x=S.economia.resumo(), o=S.ai.orcamento(), ps=S.ai.statusFornecedores(), saldo=ps.openrouter&&ps.openrouter.saldoConta;
    const resumo=$('#economiaResumoPanel'); if(!resumo)return;
    resumo.innerHTML=`
      <div class="panel-head"><span class="panel-label">Caixa da empresa</span><span class="tag tag-real">USD REAL</span></div>
      <div class="stats">
        <div class="stat"><span>Caixa disponível para IA</span><b>US$ ${Number(x.caixaUSD||0).toFixed(4)}</b></div>
        <div class="stat"><span>Saldo real no OpenRouter</span><b>${saldo!=null?'US$ '+Number(saldo).toFixed(4):'não sincronizado'}</b></div>
        <div class="stat"><span>Receita detectada</span><b>US$ ${Number(x.receitaUSD||0).toFixed(4)}</b></div>
        <div class="stat"><span>Gasto acumulado de IA</span><b>US$ ${Number(x.gastoIAUSD||0).toFixed(4)}</b></div>
        <div class="stat"><span>Vendas a identificar</span><b>US$ ${Number(x.receitaNaoIdentificadaUSD||0).toFixed(4)}</b></div>
        <div class="stat"><span>Limite de hoje</span><b>US$ ${Number(o.limiteDiarioUSD||0).toFixed(4)}</b></div>
      </div>
      <p class="panel-foot">O caixa nunca pode ser maior que o saldo disponível no provedor no momento em que é definido. Gastos reais de IA reduzem o caixa. Novos créditos detectados pela Management Key entram como receita automaticamente.</p>`;
    const produtos=(e.arquivos||[]).filter(a=>a.classe==='produto');
    const sel=$('#vendaProdutoSel'); if(sel)sel.innerHTML='<option value="">Selecione um produto (opcional)</option>'+produtos.map(a=>`<option value="${esc(a.id)}">${esc(a.nome)}</option>`).join('');
    const inp=$('#vendaValorInput'); if(inp && !inp.value && x.receitaNaoIdentificadaUSD>0) inp.value=Number(x.receitaNaoIdentificadaUSD).toFixed(4);
    const vendas=$('#vendasList'); if(vendas)vendas.innerHTML=x.vendas.length?x.vendas.slice().reverse().map(v=>`<div class="item"><div class="item-body"><div class="item-title">${esc(v.produto)}</div><div class="item-meta"><span>US$ ${Number(v.valorUSD||0).toFixed(4)} · ${F.hora(v.t)}</span></div></div></div>`).join(''):'<div class="empty">Nenhuma venda identificada ainda.</div>';
    const mov=$('#economiaMovimentos'); if(mov)mov.innerHTML=x.historico.length?x.historico.slice(-40).reverse().map(m=>`<div class="log-line"><span class="log-ts">${F.hora(m.t)}</span><span class="log-txt">${esc(m.descricao)} ${Number(m.valorUSD)!==0?'· '+(Number(m.valorUSD)>0?'+':'')+'US$ '+Number(m.valorUSD).toFixed(4):''}</span></div>`).join(''):'<div class="empty">Sem movimentações.</div>';
  }
  ...
  function pintarMotor() {
    ...
    $('#consumoPanel').innerHTML = `
      ...
      <div class="stats">
        <div class="stat"><span>Caixa atual da empresa</span><b>US$ ${Number(o.restanteUSD||0).toFixed(4)}</b></div>
        <div class="stat"><span>Gasto acumulado de IA</span><b>US$ ${Number(o.custoPeriodo||0).toFixed(4)}</b></div>
        <div class="stat"><span>Receita detectada</span><b>US$ ${Number(o.receitaUSD||0).toFixed(4)}</b></div>
        ...
        ${(o.esgotado || o.esgotadoDia || (S.ai.estado && S.ai.estado.orcamentoPreventivo)) ? `<p class="panel-foot" style="color:#E08573"><strong>${o.esgotado ? 'Caixa da empresa esgotado.' : 'Limite diário esgotado.'}</strong> A equipe não fará novas chamadas e entra em rotina Sims-like até ${o.esgotado ? 'uma nova receita ser adicionada ao provedor' : 'o próximo dia'}. ${(!o.esgotado && o.modoOrcamento !== 'intensivo') ? 'Ative Trabalho intensivo se quiser usar o saldo mensal restante hoje.' : ''}</p>` : ''}
  ...
  async function enviarReuniao(texto) {
    ...
    btn.disabled=true; btn.textContent='Gerente processando…';
    try {
  ...
  function ligar() {
    ...
    const vendaBtn=$('#registrarVendaBtn');
    if(vendaBtn) vendaBtn.onclick=()=>{try{const sel=$('#vendaProdutoSel'), livre=$('#vendaProdutoLivre'), valor=$('#vendaValorInput');S.economia.registrarVenda(sel&&sel.value,livre&&livre.value,valor&&valor.value);if(livre)livre.value='';if(valor)valor.value='';toast('Venda identificada no caixa.','ok');pintarEconomia();}catch(err){toast(err.message||'Não foi possível registrar a venda.','erro');}};
  ...
  /* ---------- reações a eventos ---------- */
  function assinar() {
    S.bus.on('ia', () => { pintarPulso(); if (viewAtual === 'motor') pintarMotor(); if(viewAtual==='economia') pintarEconomia(); });
    S.bus.on('economia', () => { if(viewAtual==='economia') pintarEconomia(); });
