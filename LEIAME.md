...
- O caixa só pode ser definido até o saldo real disponível no OpenRouter, consultado pela **Management Key**.
- Cada chamada de IA debita o custo real/estimado do caixa da empresa e o limite diário é recalculado a partir do caixa restante.
- Quando `total_credits` do OpenRouter aumenta depois da linha de base, a diferença entra automaticamente como **receita/venda a identificar**.
- Registrar uma venda apenas associa essa receita já detectada a um produto; não duplica o dinheiro.
- Se consumo externo reduzir o saldo do provedor abaixo do caixa, o caixa é automaticamente reduzido ao lastro real.
- Salários usam **Créditos internos (Cr)**, sem equivalência com dólar ou dinheiro real. A folha diária é processada localmente e não consome IA.
- O modo jogo ganhou acesso direto à Economia; a versão clássica ganhou um painel completo com caixa, receitas, vendas e movimentações.
- A sincronização continua automática: não há botão de sincronização manual.

Detalhes técnicos desta versão: `CHANGELOG.md`.

## v53 — produto final significa contato com o cliente

Entregas cliente-visíveis seguem obrigatoriamente **esboço → protótipo → candidato final → produto**. O release final é congelado e passa por um gate local que impede placeholders, notas internas, status editoriais e outros resíduos de produção. Documentos internos nunca são publicados. O ZIP de release contém apenas produtos finais; o ZIP geral é explicitamente um backup do acervo.

A sala de reuniões também respeita a autoridade do dono: ordens explícitas chegam primeiro à gerente e são materializadas quando o simulador consegue executá-las, sem transformar uma ordem em uma votação entre funcionários.
...
- **Loop de IA corrigido.** Um agente (a gerente, mas o mesmo bug podia
  ...
  Causa raiz e a trava: CHANGELOG.md.
