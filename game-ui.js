...
(function (S) {
  ...
  const SETOR_LABEL={criacao:'Produto & Criação',desenvolvimento:'Desenvolvimento de Software',producao:'Produção & Entrega',operacoes:'Operações & Dados',comercial:'Crescimento & Comercial',financeiro:'Finanças & Eficiência',laboratorio:'Laboratório & Pesquisa'};
  const rotuloPessoa=f=>f?`${f.nome} (${f.papel==='gerente'?'Gerência Geral':(SETOR_LABEL[f.especialidade]||f.cargo)+(f.liderSetor?' · líder':'')})`:'sem responsável';
  ...
  /* ---------- modal genérico ---------- */
  let fundacaoModalEmpresa=null;
  function abrirModal(html) { $('#modalCaixa').className='modal-caixa';$('#modalCaixa').innerHTML = html; $('#modal').classList.remove('oculto'); }
  function fecharModal(){if(fundacaoModalEmpresa){S.studio.descartarFundacao(fundacaoModalEmpresa);fundacaoModalEmpresa=null;}$('#modal').classList.add('oculto');$('#modalCaixa').className='modal-caixa';$('#modalCaixa').innerHTML='';pintarTudo();if(!S.state.atual()&&$('#mainMenu'))abrirMenuPrincipal();}
  ...
  function baixarConteudo(a){
    if(Array.isArray(a.pacote))S.arquivo.baixarBlob(S.arquivo.zip(a.pacote),a.nome);
    else if(/^data:/.test(a.conteudo||''))S.arquivo.baixarBlob(S.arquivo.dataUrlBlob(a.conteudo),a.nome);
    else S.arquivo.baixarBlob(new Blob([a.conteudo],{type:'text/plain;charset=utf-8'}),a.nome);
  }
  function previewPacote(a,view){
    view.innerHTML=`<label>Arquivo da edição publicada</label><select id="afArquivo">${a.pacote.map((f,i)=>`<option value="${i}">${esc(f.nome)}</option>`).join('')}</select><div id="afArquivoView"></div>`;
    const select=$('#afArquivo'),target=$('#afArquivoView');
    const paint=()=>{const f=a.pacote[Number(select.value)];target.innerHTML='';if(!f)return;
      if(/^data:image/.test(f.conteudo)){const img=document.createElement('img');img.className='artifact-preview image';img.src=f.conteudo;img.alt=f.nome;target.appendChild(img);}
      else if(/\.html?$/i.test(f.nome)){let html=f.conteudo;for(const resource of a.pacote){if(resource===f)continue;const url=/^data:/.test(resource.conteudo)?resource.conteudo:dataUrlTexto(resource.conteudo,mimeTipo(resource.tipo));for(const ref of [resource.nome,'./'+resource.nome])html=html.split('"'+ref+'"').join('"'+url+'"').split("'"+ref+"'").join("'"+url+"'");}const frame=document.createElement('iframe');frame.className='artifact-preview';frame.sandbox='allow-scripts';frame.title=f.nome;frame.srcdoc=html;target.appendChild(frame);}
      else{const pre=document.createElement('pre');pre.className='source-view';pre.textContent=f.conteudo;target.appendChild(pre);}
    };select.onchange=paint;select.value=String(Math.max(0,a.pacote.findIndex(f=>/index\.html?$/i.test(f.nome))));paint();
  }
  ...
  function pintarRail() {
    ...
    $('#railFundacao').textContent = e.fundacao && e.fundacao.estado === 'operacional'
      ...
      : e.fundacao&&e.fundacao.estado==='criando'?`Em preparação — etapa: ${(e.fundacao.etapaAtual||'estruturando').replace(/_/g,' ')}.`:`Interrompida — ${e.fundacao&&e.fundacao.ultimoErro||'aguardando retomada'}`;
  ...
  function pintarTarefas() {
    ...
    const abertas = e.tarefas.filter(t => t.status !== 'feita' && t.status !== 'descartada');
    const fazendo=e.tarefas.filter(t=>t.status==='fazendo').length,bloqueadas=e.tarefas.filter(t=>t.bloqueada&&t.status!=='descartada').length,feitas=e.tarefas.filter(t=>t.status==='feita'&&!t.consolidada).length,consolidadas=e.tarefas.filter(t=>t.consolidada).length;
    el.innerHTML = `<div class="dock-resumo"><span><b>${fazendo}</b> agora</span><span><b>${abertas.length}</b> na fila</span><span><b>${feitas}</b> concluídas</span><span class="${bloqueadas?'ruim':''}"><b>${bloqueadas}</b> bloqueadas</span>${consolidadas?`<span><b>${consolidadas}</b> duplicatas arquivadas</span>`:''}</div>`+(abertas.length ? abertas.map(t => {
      ...
      const rt=p&&S.studio.pessoa(p.id),pct=t.status==='fazendo'&&rt?Math.round(rt.progresso*100):t.bloqueada?0:5,esperaIA=Math.max(0,Number(t.retomarAposIA||0)-Date.now());return `<div class="item"><b>${esc(t.titulo)}</b><small>${t.incompleta?'INCOMPLETA · saída interrompida':t.bloqueada?'precisa de ajuste':t.status==='aguardando_decisao'?'aguardando decisão':t.status === 'fazendo' ? 'em execução' : esperaIA?`provedor em recuperação · retoma em ${S.fmt.dur(esperaIA)}`:t.motivoEsperaOrcamento?'aguardando orçamento sem gastar pensamento':'pronta para executar'} · ${esc(etapa)}${p ? ' · ' + esc(rotuloPessoa(p)) : ''} · tentativa de qualidade ${Number(t.tentativas||0)+1}</small><div class="progress" style="--p:${pct}%"><i></i></div>${t.motivoEsperaOrcamento?`<small>${esc(t.motivoEsperaOrcamento)}</small>`:''}${t.ultimoErroTransitorio?`<small>${esc(t.ultimoErroTransitorio)}</small>`:''}${t.motivoIncompleto?`<small>${esc(t.motivoIncompleto)}</small>`:''}${t.motivoEscalada?`<small>${esc(t.motivoEscalada)}</small>`:''}${t.handoff?`<small>${esc(t.handoff)}</small>`:''}${t.bloqueada||t.incompleta||t.status==='aguardando_decisao'?`<button class="mini-action" data-retomar-tarefa="${esc(t.id)}">Retomar</button>`:''}</div>`;
    }).join('') : '<div class="item"><small>Fila vazia: a gerente está definindo a próxima evolução de produto.</small></div>');
    el.querySelectorAll('[data-retomar-tarefa]').forEach(b=>b.onclick=()=>{const t=e.tarefas.find(t=>t.id===b.dataset.retomarTarefa);if(t&&S.produtos){S.produtos.retry(e,t,'Retomada solicitada pelo proprietário.');pintarTarefas();}});
  }

  /* ---------- dock: atividades ---------- */
  function pintarLog() {
    const e = S.state.atual(); const el = $('#dockLog'); if (!e) { el.innerHTML = ''; return; }
    const linhas = (e.log || []).slice().reverse(),ocorrencias=linhas.reduce((n,x)=>n+Number(x.quantidade||1),0),erros=linhas.filter(x=>x.tag==='erro').reduce((n,x)=>n+Number(x.quantidade||1),0);
    el.innerHTML = `<div class="dock-resumo"><span><b>${ocorrencias}</b> ocorrências</span><span><b>${linhas.length}</b> registros únicos</span><span><b>${erros}</b> erros</span><span><b>${e.log.filter(x=>x.tag==='trabalho').reduce((n,x)=>n+Number(x.quantidade||1),0)}</b> trabalho</span></div><div class="panel-tools"><button class="mini-action" id="logCopiar">Copiar todos</button><button class="mini-action" id="logJson">Salvar JSON no dispositivo</button></div>`+(linhas.length ? linhas.map(l => {const agente=l.agente&&(e.equipe||[]).find(f=>f.id===l.agente),vezes=Number(l.quantidade||1);return `<div class="item log-${esc(l.tag||'info')}"><span class="log-tag">${esc(l.tag||'info')}</span> ${esc(l.texto)}${vezes>1?` <b>×${vezes}</b>`:''}<small>${S.fmt.dataHora(l.t)}${l.ultimaOcorrencia&&l.ultimaOcorrencia!==l.t?' → '+S.fmt.dataHora(l.ultimaOcorrencia):''}${agente?' · '+esc(rotuloPessoa(agente)):''}</small></div>`;}).join('')
      : '<div class="item"><small>Sem atividade registrada ainda.</small></div>');
    $('#logCopiar').onclick=()=>copiarTexto((e.log||[]).map(l=>`${new Date(l.t).toISOString()} [${l.tag||'info'}] ${l.texto}${Number(l.quantidade||1)>1?` ×${l.quantidade} (até ${new Date(l.ultimaOcorrencia).toISOString()})`:''}`).join('\n'));
    $('#logJson').onclick=()=>S.arquivo.salvarNoDispositivo(new Blob([JSON.stringify({empresa:e.nome,exportadoEm:new Date().toISOString(),logs:e.log},null,2)],{type:'application/json'}),`logs-${e.id}.json`);
  }
  ...
  function pintarChat() {
    ...
    const estadoFundacao=e.fundacao&&e.fundacao.estado,etapaFundacao=e.fundacao&&e.fundacao.etapaAtual||'estruturando',fundacaoAviso=estadoFundacao==='criando'?`<div class="item"><b>Fundação em andamento · ${esc(etapaFundacao.replace(/_/g,' '))}</b><small>A ata recebe um marco a cada etapa; sem progresso por dois minutos, o watchdog interrompe e mostra o erro.</small></div>`:estadoFundacao&&estadoFundacao!=='operacional'&&estadoFundacao!=='aguardando_jogador'?`<div class="critical-card"><b>${estadoFundacao==='erro_materializacao'?'Fundação exige decisão':'Fundação aguardando retomada'}</b><small>${esc(e.fundacao.ultimoErro||'A gerente aguarda uma nova tentativa.')}</small><button class="mini-action" id="tentarFundacaoAgora">Tentar agora</button></div>`:'';
    ...
    if($('#tentarFundacaoAgora'))$('#tentarFundacaoAgora').onclick=async()=>{const b=$('#tentarFundacaoAgora');b.disabled=true;b.textContent='Tentando…';const ok=await S.studio.processarFundacaoAtual(true),falhaMaterializacao=e.fundacao&&e.fundacao.estado==='erro_materializacao';toast(ok?'Fundação concluída.':falhaMaterializacao?'A retomada explícita falhou; o motivo permanece visível e não haverá repetição automática.':'A rede ainda não respondeu; a retomada automática continua ativa.',ok?'ok':'erro');pintarTudo();};
  ...
  function abrirArtefato(id){const e=S.state.atual(),a=e&&(e.arquivos||[]).find(x=>x.id===id);if(!a)return;const p=(e.projetos||[]).find(x=>x.id===a.projectId),m=a.metricasIA||{},calls=(e.iaChamadas||[]).filter(c=>c.artifactId===a.id||(c.artifactIds||[]).includes(a.id)||c.taskId===a.taskId),isImg=['png','jpg','jpeg','webp','svg'].includes(String(a.tipo).toLowerCase()),isHtml=/^html?$/.test(String(a.tipo).toLowerCase()),site=p&&p.tipo==='site_institucional';
    ...
    const preview=()=>{if(Array.isArray(a.pacote)){previewPacote(a,view);return;}view.innerHTML='';if(isImg){const img=document.createElement('img');img.className='artifact-preview image';img.alt=a.nome;img.src=a.conteudo;view.appendChild(img);}else if(isHtml||site){const iframe=document.createElement('iframe');iframe.className='artifact-preview';iframe.setAttribute('sandbox','allow-scripts');iframe.title=`Prévia de ${a.nome}`;iframe.srcdoc=(site&&htmlPreviewProjeto(a.projectId))||String(a.conteudo||'');view.appendChild(iframe);}else{const pre=document.createElement('pre');pre.className='source-view';pre.textContent=a.conteudo;view.appendChild(pre);}};
  ...
  function pintarProdutos() {
    ...
    const progresso=(e.productRuns||[]).map(r=>{const feitas=r.pecas.filter(p=>p.status==='aceita').length,pct=Math.round(feitas/Math.max(1,r.pecas.length)*100);return `<div class="item"><b>${esc(r.nome)}</b><small>${esc(({produzindo:'Produzindo peças',revisao_final:'Revisão do pacote',liberado:'Produto liberado',precisa_ajuste:'Precisa de ajuste'})[r.status]||r.status)} · ${feitas}/${r.pecas.length} peças aceitas</small><div class="progress" style="--p:${pct}%"><i></i></div>${r.ultimoErro?`<small>${esc(r.ultimoErro)}</small>`:''}</div>`;}).join('');
    ...
    el.innerHTML = progresso+`<div class="dock-resumo"><span><b>${cont.esboco}</b> esboços</span><span><b>${cont.prototipo}</b> protótipos</span><span><b>${cont.candidato}</b> candidatos</span><span><b>${cont.produto}</b> vendáveis</span></div>${site?`<div class="item product-row critical-card"><div><b>▣ ${esc(site.nome)}</b><small>projeto obrigatório · ${siteFiles.length} arquivo(s) · ${siteFiles.some(a=>/(^|\/)index\.html?$/i.test(a.nome||''))?'prévia disponível':'em produção'}</small></div><button class="mini-action" data-ver-site="${esc(site.id)}">Abrir site</button></div>`:''}`+(itens.length ? itens.map(a => {
      const etapa = a.classe === 'produto' ? 'PRONTO PARA USO REAL' : a.classe==='referencia'?'DOCUMENTO FUNDADOR':(!a.clienteVisivel?'INTERNO · EM PRODUÇÃO':({esboco:'esboço',prototipo:'protótipo',candidato:'candidato final'}[a.classe] || a.classe));
    ...
    el.querySelectorAll('[data-baixar-produto]').forEach(btn=>btn.onclick=()=>{const a=e.arquivos.find(x=>x.id===btn.dataset.baixarProduto);if(!a)return;if(Array.isArray(a.pacote))S.arquivo.baixarBlob(S.arquivo.zip(a.pacote),a.nome);else if(['png','jpg','jpeg','webp'].includes(a.tipo)&&/^data:image\//.test(a.conteudo))S.arquivo.baixarBlob(S.arquivo.dataUrlBlob(a.conteudo),a.nome);else S.arquivo.baixarBlob(new Blob([a.conteudo],{type:'application/octet-stream'}),a.nome);S.state.registrar(`Produto baixado pelo dono: ${a.nome}.`,'download');});
  ...
  function pintarIA(){
    ...
    const media=ch.length?ch.reduce((n,x)=>n+Number(x.ms||0),0)/ch.length:0,falhas=ch.filter(x=>!x.ok).length,turno=ch.filter(x=>Date.now()-Number(x.em||0)<=6*60*60*1000);
    ...
    const totalTurno=turno.reduce((m,x)=>{m.tokens+=Number(x.tokens||0);m.custo+=Number(x.custo||0);return m;},{tokens:0,custo:0});
    const agrupar=ch.reduce((m,x)=>{const k=`${x.nivel||'legado'} · ${x.modelo||'desconhecido'}`;const v=m[k]||(m[k]={n:0,tok:0,custo:0});v.n++;v.tok+=Number(x.tokens||0);v.custo+=Number(x.custo||0);return m;},{});
    el.innerHTML=`<div class="dock-resumo"><span><b>${turno.length}</b> chamadas nas últimas 6h</span><span><b>${totalTurno.tokens}</b> tokens no turno</span><span><b>US$ ${totalTurno.custo.toFixed(6)}</b> custo no turno</span><span><b>${ch.length}</b> chamadas no histórico local</span><span><b>${total.tokens}</b> tokens históricos</span><span><b>US$ ${total.custo.toFixed(6)}</b> custo histórico</span><span><b>${media?S.fmt.dur(media):'—'}</b> latência média</span></div><div class="metric-grid">${Object.entries(agrupar).map(([k,v])=>`<div class="metric-card"><small>${esc(k)}</small><b>${v.n} chamadas</b><small>${v.tok} tokens · US$ ${v.custo.toFixed(6)}</small></div>`).join('')}</div>
      <div class="item"><b>${esc(S.ai.estado.mensagem||'Motor de IA')}</b><small>${esc(S.ai.estado.detalhe||'')} · entrada ${total.entrada} · saída ${total.saida} · ${falhas} falha(s)${bloq>0?' · provedor libera em '+S.fmt.dur(bloq):''}</small></div><div class="panel-tools"><button class="mini-action" id="iaCopiar">Copiar telemetria</button><button class="mini-action" id="iaJson">Salvar JSON no dispositivo</button></div>
      ${ch.map(x=>`<div class="item ${x.ok?'':'log-erro'}"><b>${esc(x.quem)} · ${esc(x.motivo)}</b><small>${S.fmt.dataHora(x.em)} · nível ${esc(x.nivel||'legado')} · ${esc(x.modelo)} · ${x.tokens||0} tok (${x.entrada||0}↓/${x.saida||0}↑) · US$ ${Number(x.custo||0).toFixed(6)} · ${S.fmt.dur(x.ms)} · tarefa ${esc(x.taskId||'—')} · artefato ${esc(x.artifactId||'—')} · rota: ${esc(x.rotaMotivo||'não registrada')} · ${x.ok?'ok':x.transitoria?'falha temporária; trabalho preservado · '+esc(x.erro||''):esc(x.erro||'falha')}</small></div>`).join('')||'<div class="item"><small>Nenhuma chamada persistida.</small></div>'}`;
    $('#iaCopiar').onclick=()=>copiarTexto(JSON.stringify(ch,null,2));$('#iaJson').onclick=()=>S.arquivo.salvarNoDispositivo(new Blob([JSON.stringify(ch,null,2)],{type:'application/json'}),`telemetria-ia-${e.id}.json`);
  }

  function pintarEstado(){const el=$('#dockEstado'),e=S.state.atual();if(!el||!e)return;const pessoas=S.studio.pessoas(),abertas=e.tarefas.filter(t=>t.status!=='feita'&&t.status!=='descartada'),artefatos=e.arquivos||[],eco=S.economia.resumo(),calls=e.iaChamadas||[],tokens=calls.reduce((n,c)=>n+Number(c.tokens||0),0),custo=calls.reduce((n,c)=>n+Number(c.custo||0),0),naoAtribuido=Math.max(0,Number(eco.gastoIAUSD||0)-custo),fin=(e.financeiro&&e.financeiro.analises||[]).slice(-1)[0]||{};el.innerHTML=`<div class="dock-resumo"><span><b>${pessoas.filter(p=>p.ocupado).length}/${pessoas.length}</b> trabalhando</span><span><b>${abertas.length}</b> tarefas abertas</span><span><b>${artefatos.length}</b> artefatos</span><span><b>${(e.projetos||[]).length}</b> projetos</span></div><div class="metric-grid"><div class="metric-card"><small>Caixa</small><b>US$ ${Number(eco.caixaUSD||0).toFixed(6)}</b></div><div class="metric-card"><small>Gasto IA</small><b>US$ ${Number(eco.gastoIAUSD||0).toFixed(6)}</b></div><div class="metric-card"><small>Tokens empresa</small><b>${tokens}</b></div><div class="metric-card"><small>Custo atribuído</small><b>US$ ${custo.toFixed(6)}</b></div><div class="metric-card"><small>Legado/não atribuído</small><b>US$ ${naoAtribuido.toFixed(6)}</b></div><div class="metric-card"><small>Produtos finais</small><b>${artefatos.filter(a=>a.classe==='produto').length}</b></div><div class="metric-card"><small>Custo/produto</small><b>US$ ${Number(fin.custoPorProdutoUSD||0).toFixed(6)}</b></div><div class="metric-card"><small>Tokens em release</small><b>${Number(fin.percentualTokensEmRelease||0).toFixed(1)}%</b></div><div class="metric-card"><small>Referências</small><b>${(e.acervoUsuario||[]).length}</b></div><div class="metric-card"><small>Eventos</small><b>${(e.eventos||[]).length}</b></div><div class="metric-card"><small>Falhas</small><b>${calls.filter(c=>!c.ok).length}</b></div></div><h3 class="modal-subtitulo">Funcionários ao vivo</h3>${pessoas.map(p=>`<div class="item"><b>${esc(rotuloPessoa(p.ref||p))} · ${esc(p.estado)}</b><small>${esc(p.tarefa||p.ref.foco||p.ref.pensamento||'disponível')} · energia ${Math.round(p.ref.energia||0)}% · humor ${Math.round(p.ref.humor||0)}%</small><div class="progress" style="--p:${Math.round((p.ocupado?p.progresso:0)*100)}%"><i></i></div></div>`).join('')}<h3 class="modal-subtitulo">Fila e progresso</h3>${abertas.map(t=>{const p=pessoas.find(x=>x.id===t.para),pct=t.status==='fazendo'&&p?Math.round(p.progresso*100):t.bloqueada?0:5;return`<div class="item"><b>${esc(t.titulo)}</b><small>${esc(t.status)} · ${esc(p?rotuloPessoa(p.ref||p):'sem responsável')} · ${esc(t.etapaDestino||t.escopo||'')}</small><div class="progress" style="--p:${pct}%"><i></i></div>${t.motivoEsperaOrcamento||t.motivoEscalada?`<small>${esc(t.motivoEsperaOrcamento||t.motivoEscalada)}</small>`:''}</div>`;}).join('')||'<div class="item"><small>Nenhuma tarefa aberta.</small></div>'}`;}
  ...
  function pintarStatusMundo() {
    ...
    else if(fund&&fund.estado==='erro_materializacao')$('#mundoStatus').textContent='Fundação interrompida · decisão crítica aberta';
  ...
  function pintarSelecao() {
    ...
    card.innerHTML = `<b>${esc(rotuloPessoa(p.ref||p))}</b><small>${esc(foco)}</small>`;
  ...
  function abrirPessoa(p) {
    ...
    abrirModal(`
      <span class="modal-fecha" id="pFechar">✕</span><h2>${esc(rotuloPessoa(f))}</h2>
      <div class="agent-sheet"><div class="agent-avatar">${p.papel === 'gerente' ? '♛' : (f.liderSetor?'◆':'●')}</div><div><b>${esc(p.cargo)}</b>${f.liderSetor?'<small>Líder operacional do setor · revisa etapas intermediárias, lidera projetos e pode solicitar contratações à gerente geral.</small>':''}
      ...
      <label>Agente de IA próprio</label><p class="modal-nota">Lane independente · roteador local escolhe leve ou padrão conforme a tarefa. O avançado só entra em resgate complexo ou após correções repetidas, levando o contexto integral em uma chamada robusta.</p><select id="pModeloLeve">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${(f.ia&&f.ia.leve)===m.id?'selected':''}>Leve · ${esc(m.nome)}</option>`).join('')}</select><select id="pModeloPadrao">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${(f.ia&&f.ia.padrao)===m.id?'selected':''}>Padrão · ${esc(m.nome)}</option>`).join('')}</select><select id="pModeloAvancado">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${(f.ia&&f.ia.avancado)===m.id?'selected':''}>Avançado · ${esc(m.nome)}</option>`).join('')}</select><button id="pSalvarIA" class="botao fraco">Salvar modelos deste funcionário</button>
    ...
    $('#pSalvarIA').onclick=()=>{f.ia=Object.assign({},f.ia,{leve:$('#pModeloLeve').value,padrao:$('#pModeloPadrao').value,avancado:$('#pModeloAvancado').value,independente:true});delete f.ia.pensamento;delete f.ia.producao;S.state.gravar();toast(`Três níveis próprios de ${p.nome} salvos.`,'ok');};
  }

  function abrirSalaReuniao(){
    const e=S.state.atual();if(!e)return;
    ...
    $('#srBaixar').onclick=()=>{const linha=x=>`- **${new Date(x.t||x.em||x.criadaEm||Date.now()).toISOString()} — ${x.quem||x.autor||x.titulo||'Registro'}:** ${x.texto||x.descricao||x.motivo||x.status||''}`;const ata=[`# Ata da sala de reuniões — ${e.nome}`,`Exportada em ${new Date().toISOString()}`,'','## Registro completo',...msgs.slice().reverse().map(linha),'','## Decisões críticas',...(crit.length?crit.map(linha):['- Nenhuma pendente.']),'','## Aprovações',...(aprs.length?aprs.map(linha):['- Nenhuma registrada.']),'','## Recomendações financeiras',...(frecs.length?frecs.map(linha):['- Nenhuma registrada.']),'','## Solicitações de contratação',...(pedidosContratacao.length?pedidosContratacao.map(linha):['- Nenhuma registrada.'])].join('\n');S.arquivo.salvarNoDispositivo(new Blob([ata],{type:'text/markdown;charset=utf-8'}),`ata-reuniao-${S.util.slug(e.nome)}-${new Date().toISOString().slice(0,10)}.md`);};
  ...
  async function abrirFundar() {
    ...
    const e=S.studio.iniciarFundacao(),g=(e.equipe||[]).find(x=>x.papel==='gerente'),q=e.fundacao.perguntas||{};fundacaoModalEmpresa=e;
    const caixaHtml=igual?`<p class="modal-nota">Caixa automático: o saldo global será dividido igualmente entre todas as empresas.</p>`:`<label>Forma do caixa dedicado</label><select id="fAlocacao"><option value="valor">Valor fixo em US$</option><option value="percentual">Porcentagem do saldo real</option></select><div id="fValorWrap"><label>Valor (US$)</label><input id="fCaixa" type="number" min="0" max="${Number(livre||0)}" step="0.0001" value="0"></div><div id="fPctWrap" class="oculto"><label>Porcentagem dedicada</label><input id="fPercentual" type="number" min="0" max="100" step="0.1" value="10"></div>`;
    abrirModal(`<span class="modal-fecha" id="fFechar">✕</span><h2>Conversa de fundação</h2><div class="agent-log"><div class="item"><b>${esc(g&&g.nome||'Gerente')}</b><small>Fui criada para fundar a empresa. Conte-me apenas a ideia em detalhes; depois farei as perguntas de alinhamento que realmente importam.</small></div></div><textarea id="fIdeia" placeholder="Descreva a ideia, o problema e o que você imagina construir…">${esc(q.ideia||'')}</textarea>${caixaHtml}<div class="modal-linha"><button id="okFundar" class="botao">Enviar ideia</button></div>`);
    ...
    $('#okFundar').onclick=async()=>{const b=$('#okFundar'),ideia=$('#fIdeia').value.trim();if(ideia.length<12){toast('Descreva a ideia com um pouco mais de detalhe.','erro');return;}b.disabled=true;b.textContent='A gerente está analisando…';try{
      const alocacao=igual?{tipo:'igual'}:$('#fAlocacao').value==='percentual'?{tipo:'percentual',valor:Number($('#fPercentual').value||0)}:{tipo:'valor',valor:Number($('#fCaixa').value||0)};
      if(alocacao.tipo==='igual')S.economia.distribuirIgualmente(orc.openrouterSaldoEfetivo,'Empresa provisória recebeu caixa para a conversa de fundação');else if(alocacao.tipo==='percentual')S.economia.definirCaixaPorPercentual(alocacao.valor,orc.openrouterSaldoEfetivo);else S.economia.definirCaixa(alocacao.valor,orc.openrouterSaldoEfetivo);
      const perguntas=await S.studio.perguntarAlinhamentoFundacao(e,ideia);abrirModal(`<span class="modal-fecha" id="fFechar2">✕</span><h2>Conversa de fundação</h2><div class="agent-log"><div class="item"><b>Você</b><small>${esc(ideia)}</small></div><div class="item"><b>${esc(g&&g.nome||'Gerente')}</b><small>${esc(perguntas).replace(/\n/g,'<br>')}</small></div></div><textarea id="fResposta" placeholder="Responda às perguntas na mesma mensagem…"></textarea>${S.acervo.globais().length?`<label>Referências para o primeiro projeto</label><div class="fundar-acervo">${S.acervo.globais().map(a=>`<label><input type="checkbox" name="fAcervo" value="${esc(a.id)}"> <span>${esc(a.nome)}</span></label>`).join('')}</div>`:''}<div class="modal-linha"><button id="finalizarFundar" class="botao">Responder e fundar</button></div>`);$('#fFechar2').onclick=fecharModal;$('#finalizarFundar').onclick=()=>{const resposta=$('#fResposta').value.trim();if(!resposta){toast('Responda às perguntas da gerente.','erro');return;}try{const refs=Array.from(document.querySelectorAll('[name="fAcervo"]:checked')).map(x=>x.value);S.studio.responderAlinhamentoFundacao(e,resposta,refs);fundacaoModalEmpresa=null;fecharModal();fecharMenuPrincipal();pintarTudo();toast('Empresa criada. A gerente entrou no escritório e está montando o plano de obra.','ok');void S.studio.processarFundacaoAtual(true).then(()=>pintarTudo()).catch(err=>toast(err.message||'A fundação será retomada automaticamente.','erro'));}catch(err){toast(err.message||'Falha ao iniciar a empresa.','erro');}};
    }catch(err){b.disabled=false;b.textContent='Enviar ideia';toast(err.message||'Falha ao conversar com a gerente.','erro');}};
  }
  ...
  function abrirEconomia(){
    ...
    const produtos=(e.arquivos||[]).filter(a=>a.classe==='produto'),igual=S.ai.cfg.distribuicaoCaixa==='igual',saldoEfetivo=o.openrouterSaldoEfetivo,livre=S.economia.saldoNaoAlocado(saldoEfetivo,e.id),fin=((e.financeiro&&e.financeiro.analises)||[]).slice(-1)[0]||{},cap=fin.capacidadeEquipe;
    abrirModal(`
      <span class="modal-fecha" id="eFechar">✕</span>
      <h2>Economia · ${esc(e.nome)}</h2>
      <p style="font-size:13px;line-height:1.55"><b>Caixa:</b> US$ ${Number(x.caixaUSD||0).toFixed(4)}<br><b>Saldo OpenRouter:</b> ${saldo!=null?'US$ '+Number(saldo).toFixed(4):'não sincronizado'}<br><b>Gasto IA:</b> US$ ${Number(x.gastoIAUSD||0).toFixed(4)}<br><b>Receita detectada:</b> US$ ${Number(x.receitaUSD||0).toFixed(4)}<br><b>A identificar:</b> US$ ${Number(x.receitaNaoIdentificadaUSD||0).toFixed(4)}<br><b>Limite de hoje:</b> US$ ${Number(o.limiteDiarioUSD||0).toFixed(4)}<br><b>Turno produtivo:</b> ${Number(o.turno?.decorridoHoras||0).toFixed(2)}h / ${Number(o.turno?.alvoHoras||6)}h · projeção ${Number(o.turno?.projecaoHoras||6).toFixed(1)}h<br><b>Capacidade financeira:</b> ${cap?`${cap.funcionariosAtuais}/${cap.maxFuncionarios} agentes · US$ ${Number(cap.custoTurnoPorAgente).toFixed(4)} por agente/turno`:'calculando'}</p>
      <h3 class="modal-subtitulo">Eficiência voltada ao produto</h3><p style="font-size:13px;line-height:1.55"><b>Custo por produto liberado:</b> US$ ${Number(fin.custoPorProdutoUSD||0).toFixed(6)}<br><b>Tokens que servem ao cliente:</b> ${Number(fin.percentualTokensCliente||0).toFixed(1)}%<br><b>Tokens que chegaram a release:</b> ${Number(fin.percentualTokensEmRelease||0).toFixed(1)}%<br><b>Tempo até o primeiro artefato:</b> ${fin.tempoPrimeiroArtefatoMs?Math.round(fin.tempoPrimeiroArtefatoMs/60000)+' min':'aguardando'}<br><b>Tarefas internas por produto:</b> ${Number(fin.tarefasInternasPorProduto||0).toFixed(1)}<br><b>Aproveitamento de cache:</b> ${Number(fin.aproveitamentoCache||0).toFixed(1)}%</p>
      <label>Período deste orçamento (dias)</label><div class="modal-linha"><input id="ePeriodoDias" type="number" min="1" max="365" step="1" value="${Number(x.cicloDias||30)}"><button id="eSalvarPeriodo" class="botao fraco">Aplicar período</button></div><p class="modal-nota">Exemplo: US$ 5 por 3 dias gera um teto diário inicial próximo de US$ 1,67, recalculado pelo caixa restante.</p>
      ...
      <h3 class="modal-subtitulo">Política econômica de imagens</h3><div class="acervo-duas"><div><label>Máximo por imagem (US$)</label><input id="eImgValor" type="number" min="0.001" step="0.001" value="${Number(x.imagens?.limitePorImagemUSD||0.05)}"></div><div><label>Máximo do caixa por imagem (%)</label><input id="eImgPct" type="number" min="1" max="10" step="1" value="${Math.min(10,Number(x.imagens?.percentualMaxCaixa||10))}"></div></div><button id="eSalvarImagem" class="botao fraco">Salvar política de imagens</button><p class="modal-nota">Imagens têm teto adicional de 10% do orçamento diário e exigem tarefa visual explícita executada por Produto & Criação.</p>
    ...
    $('#eSalvarPeriodo').onclick=()=>{try{S.economia.definirPeriodoDias($('#ePeriodoDias').value);toast('Período do orçamento atualizado.','ok');abrirEconomia();pintarHud();}catch(err){toast(err.message||'Falha ao atualizar período.','erro');}};
  ...
  function abrirConfig(depois) {
    ...
    abrirModal(`
      ...
      <label>Nível leve · rotina e decisões curtas</label>
      <select id="cLeve">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${cfg.leve===m.id?'selected':''}>${esc(m.nome)}</option>`).join('')}</select>
      <label>Nível padrão · produção e revisão</label>
      <select id="cPadrao">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${cfg.padrao===m.id?'selected':''}>${esc(m.nome)}</option>`).join('')}</select>
      <label>Nível avançado · resgate excepcional</label>
      <select id="cAvancado">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${cfg.avancado===m.id?'selected':''}>${esc(m.nome)}</option>`).join('')}</select>
      ...
      <p class="modal-nota">Estes modelos são o padrão para novos funcionários. Cada pessoa recebe uma cópia independente e pode ser configurada na própria ficha. O caixa e o ritmo de trabalho continuam separados por empresa.</p>
      <label>Status</label>
    ...
    $('#okConfig').onclick = async () => {
      try {
        ...
        S.ai.salvarCfg(chave || undefined, $('#cLeve').value, $('#cPadrao').value, $('#cAvancado').value, undefined, undefined, undefined, cfg.modoOrcamento, undefined, $('#cImagem').value);
  ...
  async function iniciar() {
    S.state.carregar();
    let erroStorage='';
    try{await S.state.pronto;}catch(err){erroStorage=err.message||'Falha ao recuperar os arquivos locais.';}
    S.ai.iniciar();
    if(erroStorage){S.ai.estado.pausado=true;S.ai.estado.texto=erroStorage;}
    S.studio.montar();
    ...
    if(erroStorage){$('#menuStatus').textContent=erroStorage+' Reabra o jogo para tentar novamente; a produção foi pausada para preservar os dados.';toast(erroStorage,'erro');}
  }
