...
(function (S) {
  ...
  function pintarTarefas() {
    ...
    const fazendo=e.tarefas.filter(t=>t.status==='fazendo').length,bloqueadas=e.tarefas.filter(t=>t.bloqueada).length,feitas=e.tarefas.filter(t=>t.status==='feita'&&!t.consolidada).length,consolidadas=e.tarefas.filter(t=>t.consolidada).length;
  ...
  function pintarChat() {
    ...
    const estadoFundacao=e.fundacao&&e.fundacao.estado,fundacaoAviso=estadoFundacao==='criando'?`<div class="item"><b>Fundação em andamento</b><small>A gerente está estruturando identidade, negócio, equipe e plano de obra.</small></div>`:estadoFundacao&&estadoFundacao!=='operacional'&&estadoFundacao!=='aguardando_jogador'?`<div class="critical-card"><b>${estadoFundacao==='erro_materializacao'?'Fundação exige decisão':'Fundação aguardando retomada'}</b><small>${esc(e.fundacao.ultimoErro||'A gerente aguarda uma nova tentativa.')}</small>${estadoFundacao!=='erro_materializacao'||(!e.fundacao.materializacaoEscalada&&Number(e.fundacao.tentativasMaterializacao||0)<2)?'<button class="mini-action" id="tentarFundacaoAgora">Tentar agora</button>':''}</div>`:'';
