...
(function (S) {
  ...
  function abrirPessoa(p) {
    ...
    abrirModal(`
      ...
      <label>Agente de IA próprio</label><p class="modal-nota">Lane independente · pensamento forte para decidir · produção leve para executar. Após duas reprovações, a produção escala automaticamente para o modelo de pensamento.</p><select id="pModeloPensamento">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${(f.ia&&f.ia.pensamento)===m.id?'selected':''}>Pensamento · ${esc(m.nome)}</option>`).join('')}</select><select id="pModeloProducao">${S.ai.MODELOS.map(m=>`<option value="${m.id}" ${(f.ia&&f.ia.producao)===m.id?'selected':''}>Produção · ${esc(m.nome)}</option>`).join('')}</select><button id="pSalvarIA" class="botao fraco">Salvar modelos deste funcionário</button>
    ...
    $('#pSalvarIA').onclick=()=>{f.ia=Object.assign({},f.ia,{pensamento:$('#pModeloPensamento').value,producao:$('#pModeloProducao').value,independente:true});S.state.gravar();toast(`Modelos próprios de ${p.nome} salvos.`,'ok');};
  }
