...
(function (S) {
  ...
  async function abrirFundar() {
    ...
    $('#okFundar').onclick=async()=>{const b=$('#okFundar'),ideia=$('#fIdeia').value.trim();if(ideia.length<12){toast('Descreva a ideia com um pouco mais de detalhe.','erro');return;}b.disabled=true;b.textContent='A gerente está analisando…';try{
      ...
      const perguntas=await S.studio.perguntarAlinhamentoFundacao(e,ideia);abrirModal(`<span class="modal-fecha" id="fFechar2">✕</span><h2>Conversa de fundação</h2><div class="agent-log"><div class="item"><b>Você</b><small>${esc(ideia)}</small></div><div class="item"><b>${esc(g&&g.nome||'Gerente')}</b><small>${esc(perguntas).replace(/\n/g,'<br>')}</small></div></div><textarea id="fResposta" placeholder="Responda às perguntas na mesma mensagem…"></textarea>${S.acervo.globais().length?`<label>Referências para o primeiro projeto</label><div class="fundar-acervo">${S.acervo.globais().map(a=>`<label><input type="checkbox" name="fAcervo" value="${esc(a.id)}"> <span>${esc(a.nome)}</span></label>`).join('')}</div>`:''}<div class="modal-linha"><button id="finalizarFundar" class="botao">Responder e fundar</button></div>`);$('#fFechar2').onclick=fecharModal;$('#finalizarFundar').onclick=async()=>{const fim=$('#finalizarFundar'),resposta=$('#fResposta').value.trim();if(!resposta){toast('Responda às perguntas da gerente.','erro');return;}fim.disabled=true;fim.textContent='Fundando empresa…';try{const refs=Array.from(document.querySelectorAll('[name="fAcervo"]:checked')).map(x=>x.value);S.studio.responderAlinhamentoFundacao(e,resposta,refs);await S.studio.processarFundacaoAtual(true);fecharModal();const ok=e.fundacao&&e.fundacao.estado==='operacional';toast(ok?'Empresa fundada. A gerente definiu a estratégia e montou a equipe.':'Empresa criada. A fundação retomará quando a IA estiver disponível.',ok?'ok':'info');}catch(err){fim.disabled=false;fim.textContent='Responder e fundar';toast(err.message||'Falha ao fundar a empresa.','erro');}};
  ...
  function abrirEconomia(){
    ...
    const produtos=(e.arquivos||[]).filter(a=>a.classe==='produto'),igual=S.ai.cfg.distribuicaoCaixa==='igual',saldoEfetivo=o.openrouterSaldoEfetivo,livre=S.economia.saldoNaoAlocado(saldoEfetivo,e.id),cap=((e.financeiro&&e.financeiro.analises)||[]).slice(-1)[0]?.capacidadeEquipe;
    abrirModal(`
